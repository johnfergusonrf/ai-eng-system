...
describe("AI Engineering System - Build System", () => {
    beforeAll(async () => {
        ...
        await writeFile(
            join(CONTENT_DIR, "commands", "test-command.md"),
            SAMPLE_COMMAND,
        );
        await writeFile(
            join(CONTENT_DIR, "agents", "test-agent.md"),
            SAMPLE_AGENT,
        );
        await writeFile(
    ...
    describe("File Operations", () => {
        ...
        it("should read markdown files recursively", async () => {
            ...
            const agentFiles = await getMarkdownFiles(
...
`;
    ...
    describe("Performance", () => {
        ...
        it("should handle multiple files efficiently", async () => {
            ...
            for (let i = 0; i < 10; i++) {
                const commandContent = SAMPLE_COMMAND.replace(
                    "test-command",
                    `test-command-${i}`,
                );
                await writeFile(
                    join(CONTENT_DIR, "commands", `test-command-${i}.md`),
                    commandContent,
                );
            }
