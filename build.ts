...
async function buildClaude(): Promise<void> {
    ...
    // Copy cooking-routines hooks for cross-harness safety
    const claudeCookingHooksDir = join(CLAUDE_DIR, "hooks");
    await copyCookingHookScripts(claudeCookingHooksDir);
    await copyBranchGuardHookScripts(claudeCookingHooksDir);
...
async function validateCursorOutput(cursorRoot: string): Promise<void> {
    ...
    if (!existsSync(pluginJsonPath)) {
        ...
        if (!pluginJson.skills)
            errors.push("Cursor plugin.json missing skills path");
        if (!pluginJson.agents)
            errors.push("Cursor plugin.json missing agents path");
    }
...
async function syncToLocalClaude(): Promise<void> {
    ...
    // Sync cooking-routines hooks for cross-harness safety
    await copyCookingHookScripts(claudeHooksDir);
    await copyBranchGuardHookScripts(claudeHooksDir);
...
async function getCachedCommandFiles(): Promise<string[]> {
    if (!buildCache.commandFiles) {
        buildCache.commandFiles = await getMarkdownFiles(
            join(CONTENT_DIR, "commands"),
        );
    }
...
async function getCachedAgentFiles(): Promise<string[]> {
    if (!buildCache.agentFiles) {
        buildCache.agentFiles = await getMarkdownFiles(
            join(CONTENT_DIR, "agents"),
        );
    }
